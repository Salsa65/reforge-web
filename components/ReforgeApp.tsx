...
import { MyriaAvatar, MyriaAvatarStudio } from '@/components/MyriaAvatar';
...
function AuthGate({onGuest,allowGuest=false}:{onGuest:()=>void;allowGuest?:boolean}){
  ...
  return <main className="gate ascension-gate"><Petals/><section className="gate-poster" aria-label="Reforge Ascension poster"><AscensionMark/><div className="poster-copy"><p className="eyebrow">ANGELS FALL · STORIES RISE</p><h1>REFORGE</h1><p>FROM CHAOS, CREATION.</p></div></section><section className="gate-card">
    <AscensionMark compact/><p className="eyebrow">STORY FORGE</p><h1>REFORGE</h1><p className="gate-copy">Sign in for cloud projects and collaboration, create an account, or continue as a guest with local autosave.</p>
    ...
    <p className="gate-foot">Guest work stays on this device. Account projects can sync, collaborate, and keep Myria’s project memory across sessions.</p>
  </section></main>
}
...
export default function ReforgeApp(){
  ...
  const [cloudBusy,setCloudBusy]=useState(false); const [toast,setToast]=useState(''); const [modal,setModal]=useState<'note'|'section'|'profile'|'collab'|'myria-avatar'|null>(null); const [accessDenied,setAccessDenied]=useState(false);
  ...
  <main className="main">
    ...
    <div className="workspace-grid">
      ...
      <aside className="myria glass"><div className="myria-head"><MyriaAvatar status={myriaStatus} onOpen={()=>setModal('myria-avatar')}/><div><p className="eyebrow">PERSISTENT PROJECT COMPANION</p><h2>MYRIA</h2><span className="status-line">{myriaStatus==='thinking'?'Thinking…':myriaStatus==='talking'?'Speaking…':myriaStatus==='error'?'Text service unavailable':'Observing quietly'}</span></div><button className="icon-btn" onClick={toggleMute} title={muted?'Unmute Myria':'Mute Myria'}>{muted?<VolumeX size={18}/>:<Volume2 size={18}/>}</button></div>
  ...
  {modal==='myria-avatar'&&<div className="modal-backdrop avatar-modal" onMouseDown={e=>e.target===e.currentTarget&&setModal(null)}><div className="modal avatar-modal-card"><MyriaAvatarStudio onClose={()=>setModal(null)} onSaved={()=>notify('Myria’s 3D avatar is linked on this device.')}/></div></div>}
